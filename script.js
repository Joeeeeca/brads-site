...
window.addEventListener('DOMContentLoaded', function() {
    ...
    function nextSlide() {
        ...
        // Calculate the new translateX value based on the current index and item width
        const newTranslateX = -currentIndex * itemWidth;

        // Apply the transition to the carousel to make the sliding smooth
        carousel.style.transition = 'transform 0.5s ease-in-out';

        // Apply the new transform value
        carousel.style.transform = `translateX(${newTranslateX}px)`;

        // Wait for the transition to complete before allowing the next transition
        setTimeout(() => {
            isTransitioning = false;

            // Check if the current index exceeds the original items count
            if (currentIndex >= totalItems) {
                // Reset the current index to simulate infinite looping
                currentIndex = 0;

                // Reset the transform to the beginning without transition
                carousel.style.transition = 'none';
                carousel.style.transform = `translateX(0)`;

                // Wait for a brief moment before re-enabling the transition
                setTimeout(() => {
                    carousel.style.transition = 'transform 0.5s ease-in-out';
                }, 50);
            }
